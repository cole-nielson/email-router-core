...
class ClientIdentificationResult:
    ...
    def __init__(
        ...
        """Initialize client identification result.

        Args:
            client_id: Identified client ID or None if no match
            confidence: Match confidence score (0.0-1.0)
            method: Method used for identification
            domain_used: Domain used for identification
        """
    ...
    def __repr__(self) -> str:
        """Return string representation of identification result."""
        return (
            f"ClientIdentificationResult(client_id='{self.client_id}', "
...
class EnhancedClientManager:
    ...
    def _build_comprehensive_domain_mapping(self) -> None:
        ...
        logger.info(
            f"Comprehensive domain mapping complete: {len(self._domain_to_client_cache)} domains mapped "
    ...
    def identify_client_by_domain(self, domain: str) -> ClientIdentificationResult:
        ...
        if self.enable_fuzzy_matching:
            ...
            best_match, confidence, method = self._domain_matcher.match_domain(
            ...
            if best_match and confidence >= self.confidence_threshold:
                ...
                logger.debug(
                    f"Fuzzy match: {domain} -> {best_match} -> {client_id} "
        ...
        if best_similar_domain and best_similarity >= 0.6:
            ...
            if client_id:
                logger.debug(
                    f"Similarity match: {domain} -> {best_similar_domain} -> {client_id} "
