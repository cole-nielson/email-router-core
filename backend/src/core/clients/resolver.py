...
def is_subdomain_of(subdomain: str, parent_domain: str) -> bool:
    ...
    normalized_parent = normalize_domain(parent_domain)

    if not normalized_subdomain or not normalized_parent:
        return False
...
def match_domain_pattern(domain: str, pattern: str) -> bool:
    ...
    pattern = pattern.replace(
        "*", r".*"
    )  # Changed from [^\.] to .* to match any characters including dots
    pattern = f"^{pattern}$"
...
def calculate_domain_similarity(domain1: str, domain2: str) -> float:
    ...
    Examples:
        ...
        0.8
    """
    normalized_domain1 = normalize_domain(domain1)
    normalized_domain2 = normalize_domain(domain2)

    if not normalized_domain1 or not normalized_domain2:
        return 0.0

    domain1 = normalized_domain1
    domain2 = normalized_domain2
...
def find_best_domain_match(
    target_domain: str, candidate_domains: List[str]
    ...
    if not normalized_target or not candidate_domains:
        return None, 0.0
    ...
    logger.debug(
        f"Best domain match for '{target_domain}': '{best_match}' (score: {best_score:.2f})"
    )
...
def resolve_domain_aliases(domain: str, alias_map: Dict[str, str]) -> str:
    ...
    for alias_pattern, canonical_domain in alias_map.items():
        if "*" in alias_pattern and match_domain_pattern(domain, alias_pattern):
            logger.debug(
                f"Resolved pattern alias '{domain}' -> '{canonical_domain}' (pattern: {alias_pattern})"
            )
...
class DomainMatcher:
    ...
    def add_alias(self, alias: str, canonical: str) -> None:
        ...
        normalized_alias = normalize_domain(alias)
        normalized_canonical = normalize_domain(canonical)
    ...
    def match_domain(
        ...
        normalized_domain = normalize_domain(domain)
        if not normalized_domain:
            return None, 0.0, "normalization_failed"
        domain = normalized_domain
        if not domain:
