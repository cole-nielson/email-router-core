...
class UserWithPermissions(BaseModel):
    ...
    jwt_refresh_token_hash: Optional[str] = Field(
        None, description="Refresh token hash"
    )
    ...
    permissions: List[UserPermission] = Field(
        default=[], description="User permissions"
    )
...
class UpdateUserRequest(BaseModel):
    ...
    full_name: Optional[str] = Field(
        None, description="Full name", min_length=1, max_length=200
    )
