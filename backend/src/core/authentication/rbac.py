...
class RBACService:
    ...
    @staticmethod
    def check_permission(
        user, permission: str, client_id: Optional[str] = None, raise_on_deny: bool = True
    ) -> bool:
        """
    ...
    @staticmethod
    def check_client_access(user, client_id: str, raise_on_deny: bool = True) -> bool:
        """
        ...
        """
        security_context = convert_legacy_user_to_security_context(user)
        return get_rbac_manager().check_client_access(security_context, client_id, raise_on_deny)
