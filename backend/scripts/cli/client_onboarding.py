...
from app.utils.domain_resolver import is_valid_domain_format, normalize_domain
...
class ClientOnboardingWizard:
    ...
    def _print_header(self):
        ...
        print(f"{Colors.ENDC}")
        print(
            f"{Colors.OKCYAN}Welcome! This wizard will help you set up a new client for the email router.{Colors.ENDC}"
        )
        print()
    ...
    def _print_success(self):
        ...
        print(f"{Colors.ENDC}")
        print(
            f"{Colors.OKGREEN}Client '{client_id}' has been successfully configured!{Colors.ENDC}"
        )
        print()
    ...
    def _validate_client_id(self, client_id: str) -> tuple[bool, str]:
        ...
        if not re.match(r"^client-\d{3}-[a-zA-Z0-9-]+$", client_id):
            return (
                False,
                "Client ID format should be 'client-XXX-company-name' (e.g., 'client-001-acme-corp')",
            )
    ...
    def _collect_basic_info(self):
        ...
        client_id = self._prompt("Client ID", validator=self._validate_client_id)
    ...
    def _collect_domain_config(self):
        ...
        primary_domain = self._prompt(
            "Primary domain (e.g., company.com)", validator=self._validate_domain
        )
        ...
        mailgun_domain = self._prompt(
            "Mailgun domain", primary_domain, validator=self._validate_domain
        )
    ...
    def _collect_contact_info(self):
        ...
        escalation_contact = self._prompt(
            "Escalation contact email", primary_contact, validator=self._validate_email
        )

        # Billing contact
        billing_contact = self._prompt(
            "Billing contact email", primary_contact, validator=self._validate_email
        )
    ...
    def _collect_routing_config(self):
        ...
        for category in categories:
            response_times[category] = self._prompt(
                f"{category.title()} response time", default_times[category]
            )
    ...
    def _collect_branding_config(self):
        ...
        company_name = self._prompt("Company name for emails", self.client_data["client"]["name"])

        # Email signature
        email_signature = self._prompt("Email signature", f"{company_name} Support Team")

        # Brand colors
        self._print_info("Brand colors will be used in email templates (hex format)")
        primary_color = self._prompt(
            "Primary brand color", "#667eea", validator=self._validate_hex_color
        )

        secondary_color = self._prompt(
            "Secondary brand color", "#764ba2", validator=self._validate_hex_color
        )
        ...
        self.client_data["branding"] = {
            ...
            "primary_color": (
                primary_color if primary_color.startswith("#") else f"#{primary_color}"
            ),
            "secondary_color": (
                secondary_color if secondary_color.startswith("#") else f"#{secondary_color}"
            ),
        }
    ...
    def _collect_settings_config(self):
        ...
        for setting_key, description, default in features:
            choice = self._prompt_choice(description, ["Yes", "No"], "Yes" if default else "No")
    ...
    def _validate_configuration(self):
        ...
        try:
            ...
            from backend.src.infrastructure.config.manager import get_config_manager
