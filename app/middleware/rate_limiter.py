...
class RateLimitStorage:
    ...
    def cleanup(self):
        ...
        if old_buckets or expired_ips:
            logger.info(f"Cleaned up {len(old_buckets)} old rate limit buckets and {len(expired_ips)} expired IP blocks")
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if not rate_limit_result["allowed"]:
                ...
                violation_rate = storage.get_request_rate(f"violations:{client_ip}", 300)  # 5 min window
    ...
    def _add_rate_limit_headers(self, response: Response, client_id: str):
        ...
        try:
            ...
            if sustained_bucket:
                ...
                response.headers["X-RateLimit-Remaining"] = str(int(sustained_bucket.remaining_tokens()))
                response.headers["X-RateLimit-Reset"] = str(int(time.time() + 60))
            
            if burst_bucket:
                response.headers["X-RateLimit-Burst-Limit"] = str(self.burst_limit)
                response.headers["X-RateLimit-Burst-Remaining"] = str(int(burst_bucket.remaining_tokens()))
        except Exception as e:
...
def get_rate_limit_info(client_identifier: str) -> Dict:
    ...
    try:
        ...
        return {
            ...
            "is_blocked": storage.is_blocked(client_identifier.replace("ip:", "")) if "ip:" in client_identifier else False
        }
    except Exception as e:
