...
def load_routing_rules(client_id: str, force_reload: bool = False) -> RoutingRules:
    ...
    Now loads from consolidated client-config.yaml instead of separate routing-rules.yaml.
    ...
    Raises:
        ClientLoadError: If configuration cannot be loaded or is invalid
    """
    # Load from consolidated client config instead of separate file
    client_config = load_client_config(client_id, force_reload)
    ...
    try:
        # Extract routing data from consolidated client config
        routing_data = {
            "routing": client_config.routing or {},
            "escalation": client_config.escalation,
            "backup_routing": client_config.backup_routing,
            "special_rules": client_config.special_rules,
        }
        ...
        logger.info(f"Loaded routing rules for {client_id} from consolidated config")
