...
class BrandingConfig(BaseModel):
    ...
    colors: Optional[Dict[str, Any]] = Field(
        default=None, description="Extended color configuration"
    )
...
class ResponseTimeDetail(BaseModel):
    """Enhanced response time configuration."""

    target: str = Field(..., description="Response time target (e.g., 'within 4 hours')")
    business_hours_only: bool = Field(default=True, description="Only count business hours")


class ResponseTimeConfig(BaseModel):
    """Response time commitments - supports both simple and enhanced formats."""

    support: Union[str, ResponseTimeDetail] = Field(..., description="Support response time")
    billing: Union[str, ResponseTimeDetail] = Field(..., description="Billing response time")
    sales: Union[str, ResponseTimeDetail] = Field(..., description="Sales response time")
    general: Union[str, ResponseTimeDetail] = Field(..., description="General response time")
    urgent: Optional[Union[str, ResponseTimeDetail]] = Field(
        default=None, description="Urgent response time"
    )
...
class BusinessHoursConfig(BaseModel):
    ...
    workdays: List[str] = Field(default=["monday", "tuesday", "wednesday", "thursday", "friday"])
    start_time: str = Field(default="09:00", description="Start time (24h format)")
    end_time: str = Field(default="17:00", description="End time (24h format)")


class SLAConfig(BaseModel):
    """SLA tracking and escalation configuration."""

    escalation_enabled: bool = Field(default=True, description="Enable automatic escalation")
    escalate_after_percentage: int = Field(
        default=80, description="Escalate after percentage of SLA time"
    )
    track_metrics: bool = Field(default=True, description="Track SLA metrics")
...
# Routing Models (moved before ClientConfig for proper ordering)
...
class ClientConfig(BaseModel):
    ...
    business_hours: Optional[BusinessHoursConfig] = Field(
        default=None, description="Business hours configuration"
    )
    sla: Optional[SLAConfig] = Field(default=None, description="SLA configuration")

    # Consolidated routing configuration (from routing-rules.yaml)
    routing: Optional[Dict[str, EmailStr]] = Field(
        default=None, description="Primary routing configuration"
    )
    escalation: Optional[EscalationConfig] = Field(default=None, description="Escalation rules")
    backup_routing: Optional[Dict[str, EmailStr]] = Field(
        default=None, description="Backup routing rules"
    )
    special_rules: Optional[SpecialRules] = Field(default=None, description="Special routing rules")
